...
//TODO: replace session management with encrypted cookie

const SESSION_COOKIE_NAME = 'CAS_SESSION';

// saves the current session to the session cookie
function saveSession (req, res) {
  // unencrypted for now
  res.cookie(SESSION_COOKIE_NAME, JSON.stringify(req.session), {
    secure: config(req).SECURE_COOKIE === 'true',
    httpOnly: true });
}

// load session at beginning of request
app.use((req, res, next) => {
  const sessionCookie = req.cookies[SESSION_COOKIE_NAME];
  req.session = (sessionCookie && JSON.parse(sessionCookie)) || {};

  next();
});

// CAS login endpoint
app.get('/login', requireParams(['service']), getService(), (req, res) => {
  // generate session
...
app.get('/p3/serviceValidate', requireParams(['service', 'ticket']), getService(), (req, res) => {
  // validate ticket
